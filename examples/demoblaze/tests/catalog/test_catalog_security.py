...
@pytest.mark.injection
@pytest.mark.parametrize(
    "sql_payload", ["1' OR '1'='1", "1' UNION SELECT NULL--", "999' OR 1=1--"]
)
...
def test_idor_product_access_IDOR_001(browser, base_url):
    ...
    logger.info(
        f"✓ Product enumeration check: {len(accessible)} products accessible"
    )
...
def test_path_traversal_product_images_TRAV_001(
...
def test_product_enumeration_ENUM_001(browser, base_url):
    ...
    logger.info(
        f"✓ Product enumeration: {total} total (P:{phones}, L:{laptops}, M:{monitors})"
    )
...
def test_timing_attack_product_existence_TIME_001(browser, base_url):
    ...
    logger.info(
        f"Timing: valid={valid_time:.3f}s, invalid={invalid_time:.3f}s, diff={time_diff:.3f}s"
    )
...
def test_session_fixation_catalog_browsing_SESS_001(browser, base_url):
    ...
    logger.info(
        f"Cookies: before={len(cookies_before)}, after={len(cookies_after)}"
    )
...
def test_security_headers_validation_HEAD_001(browser, base_url):
    ...
    logger.info(
        "  Recommended headers: X-Frame-Options, X-Content-Type-Options, CSP"
    )
...
def test_rate_limiting_catalog_browsing_RATE_001(browser, base_url):
    ...
    if not blocked:
        logger.warning(
            f"⚠ No rate limiting after {requests_count} rapid requests"
        )
