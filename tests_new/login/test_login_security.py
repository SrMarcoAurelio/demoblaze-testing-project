...
def test_rapid_concurrent_login_attempts_BOT_002(browser, base_url):
    ...
    if successful_attempts >= 15:
        ...
        logging.warning(f"Completed {successful_attempts}/20 concurrent requests")
        logging.warning("Standard: OWASP ASVS v5.0 Section 2.2.1")
        logging.warning("Impact: Distributed brute force attacks easier")
        logging.warning("CVSS Score: 7.5 (HIGH)")
        logging.warning("=" * 80)
        pytest.fail(f"DISCOVERED: System vulnerable to distributed brute force - {successful_attempts}/20 succeeded")
    else:
        logging.info("✓ DISCOVERED: System handles concurrent attempts appropriately")
        assert True
...
def test_concurrent_session_handling_AUTH_003(browser, base_url):
    ...
    try:
        ...
        if first_still_logged_in:
            ...
            logging.info("✓ DISCOVERED: First session invalidated when second login occurred")
...
def test_verbose_error_messages_INFO_001(browser, base_url):
    ...
    for malicious_input in malicious_inputs:
        ...
        if alert_text:
            ...
            dangerous_keywords = [
                ...
                "undefined", "null pointer", "debug", "traceback", "warning"
            ]
    ...
    if verbose_errors_found:
        ...
        logging.warning("Recommendation: Use generic error messages")
        logging.warning("=" * 80)
        pytest.fail(f"DISCOVERED: Verbose error messages - {len(verbose_errors_found)} cases")
    else:
...
def test_dangerous_http_methods_HTTP_001(browser, base_url):
    ...
    if allowed_methods:
        ...
        logging.warning("Impact: Potential for data manipulation or XST attacks")
...
def test_timing_attack_username_enumeration_TIME_001(browser, base_url):
    ...
    if time_diff > 0.5:
        ...
        logging.warning("Recommendation: Ensure constant-time authentication checks")
        logging.warning("=" * 80)
        pytest.fail(f"DISCOVERED: Timing difference of {time_diff:.3f}s allows enumeration")
    else:
        logging.info(f"✓ DISCOVERED: Timing difference minimal ({time_diff:.3f}s)")
        assert True
    ...
    try:
        ...
        if iframe_loaded:
            ...
            if has_content:
                ...
                logging.warning("Recommendation: Set X-Frame-Options: DENY or CSP frame-ancestors")
                logging.warning("=" * 80)
                pytest.fail("DISCOVERED: Site vulnerable to clickjacking - loads in iframe")
            else:
                logging.info("✓ DISCOVERED: Iframe created but blocked from loading content")
                assert True
        else:
        ...
        logging.info(f"✓ DISCOVERED: Clickjacking protection appears present - {e}")
        assert True
...
def test_remember_me_security_REMEM_001(browser, base_url):
    ...
    remember_keywords = [
        ...
        'remember', 'keep logged', 'stay logged'
    ]
    ...
    if not remember_me_exists:
        ...
        logging.info("Note: Manual security audit recommended for Remember Me implementation")
        assert True
