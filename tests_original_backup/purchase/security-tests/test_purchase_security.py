...
def test_no_rate_limiting_add_to_cart(browser):
    ...
    assert (
        requests_sent > 100
    ), f"Sent {requests_sent} requests in 1 sec - no rate limit detected"
...
def test_no_captcha_on_checkout(browser):
        ...
        try:
            ...
            try:
                browser.find_element(
                    By.XPATH,
                    "//*[contains(text(), 'CAPTCHA') or contains(text(), 'robot')]",
                )
    ...
    assert (
        successful_purchases > 5
    ), f"No CAPTCHA detected in {successful_purchases} attempts"
    ...
    for endpoint in endpoints:
        try:
            ...
            if response.status_code != 404:
                logging.error(
                    f"Exposed endpoint: {endpoint} - Status: {response.status_code}"
                )
        except:
            pass
...
def test_payment_script_integrity(browser):
    ...
    assert (
        vulnerable_scripts == 0
    ), f"Found {vulnerable_scripts} scripts without integrity checks"
...
def test_session_fixation_vulnerability(browser):
    ...
    session_before = [
        c for c in cookies_before if "session" in c["name"].lower()
    ]
    ...
    session_after = [
        c for c in cookies_after if "session" in c["name"].lower()
    ]
...
def test_cookie_security_flags(browser):
    ...
    for cookie in cookies:
        if "session" in cookie.get("name", "").lower():
            assert cookie.get(
                "httpOnly", False
            ), f"Cookie {cookie['name']} missing HttpOnly"
            assert cookie.get(
                "secure", False
    ...
    for method in dangerous_methods:
        try:
            ...
            if response.status_code not in [405, 501]:
                logging.error(
                    f"Method {method} allowed: {response.status_code}"
                )
        except:
            pass
