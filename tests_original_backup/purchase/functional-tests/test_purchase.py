...
def fill_order_form(
    browser, name="", country="", city="", card="", month="", year=""
):
    try:
        WebDriverWait(browser, TIMEOUT).until(
...
def test_successful_purchase_and_price_verification(order_modal_page):
    ...
    try:
        ...
        assert (
            amount_match
        ), f"Could not find amount in confirmation: {confirm_text}"
...
def test_purchase_as_logged_in_user(browser):
    ...
    fill_order_form(
        browser, "Logged User", "Spain", "Madrid", "9876543210", "06", "2029"
    )
...
def test_order_modal_close_button(order_modal_page):
    ...
    assert browser.find_element(
        *PLACE_ORDER_BUTTON
    ).is_displayed(), "Should be back on cart page"
...
def test_add_same_product_multiple_times(browser):
    ...
    logging.info(
        f"Same product added twice: {len(cart_items)} items, total: {total}"
    )
...
def test_navigation_after_purchase(order_modal_page):
    ...
    fill_order_form(
        browser, "QA Tester", "Spain", "Barcelona", "1234567890", "12", "2028"
    )
    browser.find_element(*PURCHASE_BUTTON).click()
...
def test_cart_empty_after_purchase(order_modal_page):
    ...
    assert (
        len(cart_items) == 0
    ), f"Cart should be empty after purchase, but has {len(cart_items)} items"
...
def test_add_many_products_to_cart(browser):
    ...
    logging.info(
        "TC-PURCH-018: Starting boundary test - adding 10 products..."
    )
    browser.get(BASE_URL)
...
def test_delete_all_items_from_cart(browser):
    ...
    for i in range(initial_count):
        ...
        WebDriverWait(browser, TIMEOUT).until(
            lambda d: len(
                d.find_elements(By.XPATH, "//tbody[@id='tbodyid']/tr")
            )
            == initial_count - (i + 1)
        )
    ...
    assert (
        len(final_items) == 0
    ), f"Cart should be empty, but has {len(final_items)} items"
...
def test_open_close_order_modal_multiple_times(cart_page):
    ...
    logging.info(
        "TC-PURCH-020: Starting open/close modal multiple times test..."
    )
    browser = cart_page
...
def test_access_empty_cart(browser):
    ...
    assert (
        len(cart_items) == 0
    ), f"Cart should be empty, but has {len(cart_items)} items"
...
def test_cart_persistence_across_navigation(browser):
    ...
    assert (
        len(cart_items) == 1
    ), f"Cart should persist 1 item, got {len(cart_items)}"

    total = wait_for_cart_total_update(browser)
    assert (
        total == price1
    ), f"Cart total should persist. Expected: {price1}, Got: {total}"
...
def test_place_order_button_disabled_during_processing(order_modal_page):
    ...
    logging.info(
        "TC-PURCH-028: Starting button disabled during processing test..."
    )
    browser = order_modal_page

    fill_order_form(
        browser, "Test User", "Spain", "Barcelona", "1234567890", "12", "2028"
    )
...
def test_cart_with_browser_back_button(browser):
    ...
    assert (
        len(cart_items) == 1
    ), f"Cart should have 1 item after back button, got {len(cart_items)}"
...
def test_cart_with_maximum_quantity_simulation(browser):
    ...
    logging.info(
        f"Cart handled {items_to_add} items successfully, total: {total_actual}"
    )
...
def test_order_modal_escape_key(cart_page):
    ...
    logging.info(
        f"TC-PURCH-031: PASSED (ESC {'works' if modal_closed else 'documented'})"
    )
...
def test_browser_refresh_on_order_modal(order_modal_page):
    ...
    assert browser.find_element(
        *PLACE_ORDER_BUTTON
    ).is_displayed(), "Should be on cart page after refresh"
...
def test_cart_after_logout(browser):
    ...
    cart_items_after = browser.find_elements(
        By.XPATH, "//tbody[@id='tbodyid']/tr"
    )
    ...
    if items_count_after == 0:
        ...
        logging.info(
            f"Cart persisted after logout ({items_count_after} items)"
        )
...
def test_purchase_confirmation_details(order_modal_page):
    ...
    fill_order_form(
        browser, test_name, "Spain", "Barcelona", test_card, "12", "2028"
    )
...
def test_order_form_tab_navigation(order_modal_page):
    ...
    filled_name = browser.find_element(*ORDER_NAME_FIELD).get_attribute(
        "value"
    )
...
def test_rapid_purchase_attempts(cart_page):
    ...
    fill_order_form(
        browser, "Rapid Test", "Spain", "Madrid", "1234567890", "12", "2028"
    )
...
def test_homepage_pagination(browser):
    ...
    first_product_name = (
        ...
        .until(EC.visibility_of_element_located(FIRST_PRODUCT_LINK))
        .text
    )
    ...
    WebDriverWait(browser, TIMEOUT).until(
        lambda d: d.find_element(*FIRST_PRODUCT_LINK).text
        != first_product_name
    )
    ...
    first_product_page_1_return = browser.find_element(
        *FIRST_PRODUCT_LINK
    ).text
...
def test_contact_modal_send_valid_message(browser):
    ...
    browser.find_element(*CONTACT_MESSAGE_FIELD).send_keys(
...
def test_about_us_modal_opens_and_closes(browser):
    ...
    close_button = browser.find_element(
        By.XPATH, "//div[@id='videoModal']//button[text()='Close']"
    )
    close_button.click()
...
@pytest.mark.parametrize(
    "test_id,name,country,city,card,month,year,expected_alert",
    [
        (
            "TC-PURCH-004",
            "",
            ...
            "",
            "",
            "Please fill out Name and Credit Card.",
        ),
        (
            "TC-PURCH-005",
            "QA Tester",
        ...
        (
            ...
            "12",
            "2028",
            "Invalid credit card format. Must be numeric.",
        ),
        (
            "TC-PURCH-008",
            "Test",
            ...
            "abc",
            "def",
            "Invalid date format. Month/Year must be numeric.",
        ),
        (
            "TC-PURCH-009",
            "a" * 200,
        ...
        (
            ...
            "12",
            "2028",
            "Invalid input detected. Malicious characters found.",
        ),
        (
            "TC-PURCH-011",
            "Test",
        ...
        (
            ...
            "12",
            "2028",
            "Name field cannot be only whitespace.",
        ),
        (
            "TC-PURCH-VAL-002",
            "Test",
        ...
        (
            ...
            "12",
            "2028",
            "Invalid credit card length. Must be 16 digits.",
        ),
        (
            "TC-PURCH-VAL-004",
            "Test",
...
def test_order_form_validation_robustness_security(
    order_modal_page,
    test_id,
    name,
    country,
...
def test_empty_cart_purchase_blocked(browser):
    ...
    logging.error(
        f"BUSINESS RULE VIOLATION: Expected '{expected_alert}', Got '{alert_text}'"
    )
...
def test_card_format_must_be_numeric(order_modal_page):
    ...
    fill_order_form(
        browser, "Test", "Spain", "Madrid", "ABCD-1234", "12", "2028"
    )
    browser.find_element(*PURCHASE_BUTTON).click()
    ...
    logging.error(
        f"PCI-DSS VIOLATION: Expected '{expected}', Got '{alert_text}'"
    )
...
def test_card_expiration_validation(order_modal_page):
    ...
    logging.error(
        f"BUSINESS RULE VIOLATION: Expected '{expected}', Got '{alert_text}'"
    )
...
def test_month_range_validation(order_modal_page):
    ...
    fill_order_form(
        browser, "Test", "Spain", "Madrid", "1234567890123456", "13", "2028"
    )
    browser.find_element(*PURCHASE_BUTTON).click()
...
def test_sql_injection_protection(order_modal_page):
    ...
    logging.error(
        f"OWASP VIOLATION: Expected '{expected}', Got '{alert_text}'"
    )
    logging.error(
        "Impact: SQL Injection vulnerability - CRITICAL SECURITY RISK"
    )
...
def test_name_max_length(order_modal_page):
    ...
    fill_order_form(
        browser, "a" * 200, "Spain", "Madrid", "1234567890123456", "12", "2028"
    )
    browser.find_element(*PURCHASE_BUTTON).click()
...
def test_whitespace_only_name(order_modal_page):
    ...
    fill_order_form(
        browser, "     ", "Spain", "Madrid", "1234567890123456", "12", "2028"
    )
    browser.find_element(*PURCHASE_BUTTON).click()
    ...
    logging.error(
        f"DATA QUALITY ISSUE: Expected '{expected}', Got '{alert_text}'"
    )
...
def test_contact_empty_fields(browser):
    ...
    browser.find_element(*CONTACT_NAV_LINK).click()
    WebDriverWait(browser, TIMEOUT).until(
        EC.visibility_of_element_located(CONTACT_SEND_BUTTON)
    )
    browser.find_element(*CONTACT_SEND_BUTTON).click()
