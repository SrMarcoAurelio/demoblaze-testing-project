...
def test_valid_signup_with_unique_credentials_FUNC_001(browser):
    ...
    logging.info("=" * 80)
    logging.info(
        "TC-SIGNUP-FUNC-001: Testing valid signup with unique credentials"
    )
    ...
    if alert_text and (
        "success" in alert_text.lower() or "signed up" in alert_text.lower()
    ):
        logging.info(
            f"DISCOVERED: Signup successful for user '{unique_username}'"
        )
        ...
        if logged_in:
            ...
            logging.warning(
                "Signup succeeded but cannot log in with new credentials"
            )
...
def test_duplicate_username_rejected_FUNC_002(browser):
    ...
    if second_alert and (
        "exist" in second_alert.lower()
        or "already" in second_alert.lower()
        or "taken" in second_alert.lower()
    ):
        logging.info(
            f"DISCOVERED: Duplicate username correctly rejected: {second_alert}"
        )
        assert True
    else:
        logging.error(
            f"DISCOVERED: SECURITY ISSUE - Duplicate username accepted or no clear error"
        )
...
def test_empty_credentials_rejected_FUNC_003(browser):
    ...
    if alert_text and (
        "fill" in alert_text.lower()
        or "required" in alert_text.lower()
        or "empty" in alert_text.lower()
    ):
        logging.info(
            f"DISCOVERED: Empty credentials rejected with message: {alert_text}"
        )
        assert True
    elif alert_text:
        logging.warning(
            f"DISCOVERED: Alert appeared but message unclear: {alert_text}"
        )
        assert True
    else:
...
def test_username_max_length_BR_001(browser):
    ...
    if alert_text:
        logging.info(
            f"DISCOVERED: System response to 500-char username: {alert_text}"
        )
...
def test_password_max_length_BR_002(browser):
    ...
    logging.info("=" * 80)
    logging.info(
        "TC-SIGNUP-BR-002: NIST 800-63B - Testing password max length"
    )
    ...
    if alert_text:
        logging.info(
            f"DISCOVERED: System response to 500-char password: {alert_text}"
        )
...
def test_username_leading_trailing_whitespace_BR_003(browser):
    ...
    if alert_text and "success" in alert_text.lower():
        ...
        if is_user_logged_in(browser):
            logging.info(
                "DISCOVERED: System trims whitespace (can login with trimmed version)"
            )
        else:
            logging.info(
                "DISCOVERED: System preserves whitespace (requires exact match)"
            )
...
def test_password_whitespace_significance_BR_004(browser):
    ...
    logging.info("=" * 80)
    logging.info(
        "TC-SIGNUP-BR-004: NIST 800-63B - Testing password whitespace"
    )
...
def test_special_characters_in_username_BR_005(browser):
    ...
    if alert_text:
        logging.info(
            f"DISCOVERED: System response to special chars: {alert_text}"
        )
...
def test_username_whitespace_normalization_BR_008(browser):
    ...
    logging.info("=" * 80)
    logging.info(
        "TC-SIGNUP-BR-008: ISO 25010 - Testing whitespace normalization"
    )
    ...
    if second_alert and "exist" in second_alert.lower():
        logging.info(
            "DISCOVERED: System trims whitespace (duplicate detected)"
        )
    else:
...
def test_identical_username_password_BR_010(browser):
    ...
    logging.info("=" * 80)
    logging.info(
        "TC-SIGNUP-BR-010: NIST 800-63B - Testing identical username/password"
    )
    ...
    if alert_text and "success" in alert_text.lower():
        logging.warning(
            "DISCOVERED: System allows identical username/password"
        )
...
def test_sql_injection_prevention_BR_011(browser, sql_payload):
    ...
    logging.info("=" * 80)
    logging.info(
        f"TC-SIGNUP-BR-011: OWASP ASVS 1.2.5 - SQL injection: {sql_payload}"
    )
...
def test_xss_prevention_BR_012(browser, xss_payload):
    ...
    logging.info("=" * 80)
    logging.info(
        f"TC-SIGNUP-BR-012: OWASP ASVS 1.4.1 - XSS payload: {xss_payload}"
    )
...
@pytest.mark.parametrize("weak_password", ["123456", "password", "12345678"])
def test_password_complexity_enforcement_BR_013(browser, weak_password):
    ...
    logging.info("=" * 80)
    logging.info(
        f"TC-SIGNUP-BR-013: NIST 800-63B - Password complexity: {weak_password}"
    )
...
def test_signup_rate_limiting_BR_014(browser):
    ...
    logging.info("=" * 80)
    logging.info(
        "TC-SIGNUP-BR-014: OWASP ASVS 2.2.1 - Testing signup rate limiting"
    )
    ...
    for attempt in range(max_attempts):
        ...
        if alert_text and (
            ...
            logging.info(
                f"DISCOVERED: Rate limiting triggered after {attempt + 1} attempts"
            )
            rate_limited = True
            break
    ...
    if not rate_limited:
        logging.critical("=" * 80)
        logging.critical(
            "SECURITY VIOLATION: NO SIGNUP RATE LIMITING DETECTED"
        )
        logging.critical("Standard: OWASP ASVS v5.0 Section 2.2.1")
        logging.critical("Severity: HIGH")
        logging.critical("CVSS Score: 7.5")
        logging.critical(
            f"Impact: Completed {max_attempts} signups rapidly without limit"
        )
        logging.critical(
            "Recommendation: Implement rate limiting (e.g. 5 per IP per hour)"
        )
        logging.critical("=" * 80)

        pytest.fail(
            f"DISCOVERED: No rate limiting detected after {max_attempts} attempts"
        )
...
def test_captcha_protection_BR_015(browser):
    ...
    logging.info("=" * 80)
    logging.info(
        "TC-SIGNUP-BR-015: OWASP ASVS 2.2.3 - Testing CAPTCHA protection"
    )
    ...
    if not captcha_found:
        ...
        logging.critical(
            "Impact: Automated bots can create unlimited accounts"
        )
        logging.critical(
            "Recommendation: Implement CAPTCHA (reCAPTCHA v3 recommended)"
        )
        logging.critical("=" * 80)
...
def test_email_verification_requirement_BR_016(browser):
    ...
    logging.info("=" * 80)
    logging.info(
        "TC-SIGNUP-BR-016: OWASP ASVS 2.1.12 - Testing email verification"
    )
    ...
    if signup_alert and "success" in signup_alert.lower():
        ...
        if is_user_logged_in(browser):
            logging.critical("=" * 80)
            logging.critical(
                "SECURITY VIOLATION: NO EMAIL VERIFICATION REQUIRED"
            )
            logging.critical("Standard: OWASP ASVS v5.0 Section 2.1.12")
            logging.critical("Severity: MEDIUM")
            logging.critical("CVSS Score: 5.3")
            logging.critical(
                "Impact: Users can immediately use unverified accounts"
            )
            logging.critical(
            ...
            perform_logout(browser)
            pytest.fail(
                "DISCOVERED: Account usable immediately without email verification"
            )
...
def test_keyboard_navigation_BR_017(browser):
    ...
    logging.info("=" * 80)
    logging.info(
        "TC-SIGNUP-BR-017: WCAG 2.1 SC 2.1.1 - Testing keyboard navigation"
    )
...
def test_username_enumeration_via_signup_BR_019(browser):
    ...
    logging.info("=" * 80)
    logging.info(
        "TC-SIGNUP-BR-019: OWASP ASVS 2.2.2 - Testing username enumeration"
    )
