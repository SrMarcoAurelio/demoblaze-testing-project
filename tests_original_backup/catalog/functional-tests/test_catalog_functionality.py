...
PRODUCT_DETAIL_IMAGE = (
    By.CSS_SELECTOR,
    ".product-image img, img.img-thumbnail",
)
...
def test_navigate_to_phones_category_FUNC_001(browser):
    ...
    if not success:
        pytest.fail(
            "DISCOVERED: Phones category link not clickable or products failed to load"
        )
    ...
    if not products:
        pytest.fail(
            "DISCOVERED: No products displayed after navigating to Phones"
        )
...
def test_navigate_to_laptops_category_FUNC_002(browser):
    ...
    if not success:
        pytest.fail(
            "DISCOVERED: Laptops category link not clickable or products failed to load"
        )
    ...
    if not products:
        pytest.fail(
            "DISCOVERED: No products displayed after navigating to Laptops"
        )

    logging.info(
        f"DISCOVERED: Laptops category shows {len(products)} products"
    )
    assert len(products) > 0
...
def test_navigate_to_monitors_category_FUNC_003(browser):
    ...
    if not success:
        pytest.fail(
            "DISCOVERED: Monitors category link not clickable or products failed to load"
        )
    ...
    if not products:
        pytest.fail(
            "DISCOVERED: No products displayed after navigating to Monitors"
        )

    logging.info(
        f"DISCOVERED: Monitors category shows {len(products)} products"
    )
    assert len(products) > 0
...
def test_home_button_shows_all_products_FUNC_004(browser):
    ...
    if home_count <= category_count:
        logging.warning(
            f"Home shows {home_count} products, Phones shows {category_count}"
        )
        logging.warning(
            "Expected home to show all products (more than single category)"
        )
...
def test_category_switches_correctly_FUNC_005(browser):
    ...
    phones_first_product = (
        get_product_details_from_card(browser, phones_products[0])
        if phones_products
        else None
    )
    ...
    laptops_first_product = (
        get_product_details_from_card(browser, laptops_products[0])
        if laptops_products
        else None
    )
    ...
    if not phones_first_product or not laptops_first_product:
        pytest.fail(
            "DISCOVERED: Could not extract product details from categories"
        )

    # Products should be different
    if phones_first_product["name"] == laptops_first_product["name"]:
        logging.warning(
            "DISCOVERED: Same product shown in different categories"
        )
...
def test_product_names_visible_FUNC_007(browser):
    ...
    if products_without_names:
        pytest.fail(
            f"DISCOVERED: {len(products_without_names)} products lack visible names (positions: {products_without_names})"
        )

    logging.info(
        f"DISCOVERED: All {len(products)} products have visible names"
    )
    assert True
...
def test_product_prices_visible_FUNC_008(browser):
    ...
    if products_without_prices:
        pytest.fail(
            f"DISCOVERED: {len(products_without_prices)} products lack visible prices (positions: {products_without_prices})"
        )

    logging.info(
        f"DISCOVERED: All {len(products)} products have visible prices"
    )
    assert True
...
def test_product_images_load_FUNC_009(browser):
    ...
    if products_without_images:
        pytest.fail(
            f"DISCOVERED: {len(products_without_images)} products lack image src (positions: {products_without_images})"
        )

    logging.info(
        f"DISCOVERED: All {len(products)} products have images with src attribute"
    )
    assert True
...
def test_pagination_next_button_functionality_FUNC_010(browser):
    ...
    first_product_before = (
        get_product_details_from_card(browser, products_before[0])
        if products_before
        else None
    )
    ...
    first_product_after = (
        get_product_details_from_card(browser, products_after[0])
        if products_after
        else None
    )
    ...
    if not first_product_before or not first_product_after:
        pytest.fail(
            "DISCOVERED: Could not extract product details before/after pagination"
        )

    # Products should change
    if first_product_before["name"] == first_product_after["name"]:
        pytest.fail(
            "DISCOVERED: Next button exists but products didn't change"
        )

    logging.info(
        "DISCOVERED: Next button successfully changes displayed products"
    )
    assert True
...
def test_pagination_previous_button_functionality_FUNC_011(browser):
    ...
    first_product_page2 = (
        get_product_details_from_card(browser, products_page2[0])
        if products_page2
        else None
    )
    ...
    if not prev_buttons:
        logging.warning(
            "DISCOVERED: Next button exists but no Previous button"
        )
        assert True
        return
    ...
    first_product_page1 = (
        get_product_details_from_card(browser, products_page1[0])
        if products_page1
        else None
    )
    ...
    if not first_product_page2 or not first_product_page1:
        pytest.fail(
            "DISCOVERED: Could not extract product details during pagination"
        )

    # Should return to different products
    if first_product_page2["name"] == first_product_page1["name"]:
        pytest.fail(
            "DISCOVERED: Previous button exists but didn't return to previous products"
        )

    logging.info(
        "DISCOVERED: Previous button successfully returns to previous page"
    )
    assert True
...
def test_pagination_boundary_conditions_FUNC_012(browser):
    ...
    if prev_buttons:
        ...
        if prev_button.is_displayed() and prev_button.is_enabled():
            logging.info(
                "DISCOVERED: Previous button is enabled on first page"
            )
            prev_button.click()
            time.sleep(2)
            ...
            logging.info(
                "DISCOVERED: Previous button disabled/hidden on first page (good UX)"
            )
    else:
...
def test_click_product_navigates_to_details_FUNC_013(browser):
    ...
    if current_url == BASE_URL or "prod.html" not in current_url:
        pytest.fail(
            f"DISCOVERED: Product click didn't navigate to details page. URL: {current_url}"
        )
...
def test_product_url_changes_correctly_FUNC_014(browser):
    ...
    if "idp_" not in current_url and "id=" not in current_url:
        logging.warning(
            f"DISCOVERED: Product URL may lack identifier: {current_url}"
        )
...
def test_back_button_returns_to_catalog_FUNC_015(browser):
    ...
    if current_url != initial_url:
        logging.warning(
            f"DISCOVERED: Back button didn't return to exact URL. Expected: {initial_url}, Got: {current_url}"
        )
    ...
    if not products_after_back:
        pytest.fail(
            "DISCOVERED: No products visible after browser back button"
        )

    logging.info(
        "DISCOVERED: Browser back button returns to catalog successfully"
    )
    assert True
...
def test_all_products_have_name_BR_001(browser):
    ...
    if products_without_name:
        ...
        logging.error(
            f"Affected positions: {', '.join(products_without_name)}"
        )
        ...
        pytest.fail(
            f"DISCOVERED: {len(products_without_name)} products lack names - Violates ISO 25010"
        )

    logging.info(
        f"All {len(products)} products have names - ISO 25010 compliant"
    )
    assert True
...
def test_all_products_have_price_BR_002(browser):
    ...
    for idx, product in enumerate(products, 1):
        ...
        if (
            not details
            or not details["price"]
            or len(details["price"].strip()) == 0
        ):
    ...
    if products_without_price:
        ...
        logging.error("Severity: CRITICAL")
        logging.error(
            f"Products without prices: {len(products_without_price)}"
        )
        logging.error(
            f"Affected positions: {', '.join(products_without_price)}"
        )
        logging.error("Impact:")
        ...
        pytest.fail(
            f"DISCOVERED: {len(products_without_price)} products lack prices - Violates ISO 25010"
        )

    logging.info(
        f"All {len(products)} products have prices - ISO 25010 compliant"
    )
    assert True
...
def test_all_products_have_description_BR_003(browser):
    ...
    logging.info("=" * 80)
    logging.info(
        "TC-CATALOG-BR-003: Validating all products have descriptions"
    )
    ...
    for product in products[:5]:  # Test first 5 for performance
        ...
        if details and details["link"]:
            product_links.append(
                {"name": details["name"], "url": details["link"]}
            )
    ...
    for product_data in product_links:
        ...
        try:
            ...
            description_elements = browser.find_elements(
            ...
            if not description_elements:
                ...
                logging.warning(
                    f"Product '{product_data['name']}' has no description element"
                )
                continue
            ...
            if not description_text or len(description_text) < 10:
                products_without_description.append(product_data["name"])
                logging.warning(
                    f"Product '{product_data['name']}' has insufficient description: '{description_text}'"
                )
    ...
    if products_without_description:
        ...
        logging.error(
            f"Products without adequate descriptions: {len(products_without_description)}"
        )
        logging.error(
            f"Affected products: {', '.join(products_without_description[:5])}"
        )
        logging.error("Impact:")
        ...
        logging.error(
            "Requirement: All products MUST have descriptions (minimum 10 characters)"
        )
        logging.error("=" * 80)

        pytest.fail(
            f"DISCOVERED: {len(products_without_description)} products lack descriptions - Violates ISO 25010"
        )

    logging.info(
        f"All {len(product_links)} tested products have descriptions - ISO 25010 compliant"
    )
    assert True
...
def test_all_products_have_valid_image_BR_004(browser):
    ...
    for idx, product in enumerate(
        ...
        if details and details["image_src"]:
            ...
            if not check_image_loads(image_url):
                ...
                logging.warning(
                    f"Product '{details['name']}' has broken image: {image_url}"
                )
    ...
    if products_with_broken_images:
        ...
        logging.error(
            f"Products with broken images: {len(products_with_broken_images)}"
        )
        ...
        logging.error(
            "Requirement: All product images MUST load successfully (HTTP 200)"
        )
        logging.error("=" * 80)

        pytest.fail(
            f"DISCOVERED: {len(products_with_broken_images)} products have broken images - Violates ISO 25010"
        )

    logging.info(
        "All tested product images load successfully - ISO 25010 compliant"
    )
    assert True
...
def test_price_format_consistency_BR_005(browser):
    ...
    if inconsistent_prices:
        ...
        for item in inconsistent_prices[:3]:
            logging.error(
                f"  - {item['name']}: '{item['price']}' ({item['issue']})"
            )
        logging.error("Impact:")
        ...
        pytest.fail(
            f"DISCOVERED: {len(inconsistent_prices)} prices have inconsistent format - Violates ISO 25010"
        )

    logging.info(
        f"All {len(prices)} prices follow consistent format - ISO 25010 compliant"
    )
    assert True
...
def test_product_links_not_broken_BR_006(browser):
    ...
    if broken_links:
        ...
        pytest.fail(
            f"DISCOVERED: {len(broken_links)} broken product links - Violates ISO 25010"
        )
...
def test_catalog_load_time_performance_BR_007(browser):
    ...
    if load_time > 3.0:
        ...
        logging.error(
            "Recommendation: Optimize images, implement lazy loading, use CDN"
        )
        logging.error("=" * 80)

        pytest.fail(
            f"DISCOVERED: Catalog load time {load_time:.2f}s exceeds 3s limit - Violates ISO 25010"
        )
...
def test_category_switch_response_time_BR_008(browser):
    ...
    logging.info(
        f"DISCOVERED: Category switch time: {switch_time:.2f} seconds"
    )
    ...
    if switch_time > 2.0:
        ...
        logging.error(
            "Recommendation: Implement client-side filtering or optimize AJAX calls"
        )
        logging.error("=" * 80)

        pytest.fail(
            f"DISCOVERED: Category switch {switch_time:.2f}s exceeds 2s limit - Violates ISO 25010"
        )

    logging.info(
        f"Category switches in {switch_time:.2f}s - ISO 25010 compliant"
    )
    assert True
...
def test_pagination_required_for_large_catalogs_BR_009(browser):
    ...
    logging.info(
        f"DISCOVERED: {product_count} products displayed on single page"
    )
    ...
    if product_count > 15 and not has_pagination:
        ...
        logging.error(
            "Expected: Pagination controls for catalogs with >15 items"
        )
        ...
        logging.error(
            "Industry Best Practice: Amazon, eBay use 24-48 items per page with pagination"
        )
        logging.error("=" * 80)

        pytest.fail(
            f"DISCOVERED: {product_count} products without pagination - Violates ISO 25010 and ISO 9241-110"
        )

    if has_pagination:
        logging.info(
            f"DISCOVERED: Pagination controls present for {product_count} products - Standards compliant"
        )
    else:
        logging.info(
            f"DISCOVERED: {product_count} products (acceptable without pagination)"
        )
...
def test_empty_categories_not_allowed_BR_010(browser):
    ...
    for category_name, category_locator in categories_to_test:
        ...
        logging.info(
            f"DISCOVERED: {category_name} has {product_count} products"
        )
    ...
    if empty_categories:
        ...
        pytest.fail(
            f"DISCOVERED: {len(empty_categories)} empty categories - Violates ISO 25010"
        )
...
def test_category_active_state_indication_BR_011(browser):
    ...
    try:
        ...
        if not has_active_indicator:
            ...
            logging.warning(
                "Recommendation: Add 'active' class with distinct styling"
            )
            logging.warning("=" * 80)

            # This is LOW priority, so we log warning but don't fail
            logging.info(
                "DISCOVERED: Active category indication may be missing"
            )
        else:
            logging.info(
                "DISCOVERED: Active category has visual indication - Standards compliant"
            )

    except NoSuchElementException:
        logging.warning(
            "Could not find category element to check active state"
        )
...
def test_product_images_have_alt_text_BR_012(browser):
        ...
        if details:
            ...
            if not alt_text or len(alt_text.strip()) == 0:
                images_without_alt.append(
                    {"position": idx, "name": details["name"]}
                )
    ...
    if images_without_alt:
        ...
        logging.error(
            "  - Violates legal accessibility requirements (ADA, Section 508)"
        )
        ...
        logging.error(
            "Legal Risk: HIGH - This is a Level A requirement (mandatory)"
        )
        logging.error("=" * 80)

        pytest.fail(
            f"DISCOVERED: {len(images_without_alt)} images lack alt text - Violates WCAG 2.1 Level A (MANDATORY)"
        )

    logging.info(
        f"All {len(products)} product images have alt text - WCAG 2.1 compliant"
    )
    assert True
...
def test_keyboard_navigation_categories_BR_013(browser):
    ...
    try:
        ...
        if not products:
            ...
            pytest.fail(
                "DISCOVERED: Keyboard navigation failed - Violates WCAG 2.1 Level A (MANDATORY)"
            )
...
def test_category_links_have_aria_labels_BR_014(browser):
    ...
    if links_without_proper_labels:
        ...
        logging.error("Severity: MEDIUM")
        logging.error(
            f"Links without labels: {', '.join(links_without_proper_labels)}"
        )
        logging.error("Impact:")
        ...
        pytest.fail(
            f"DISCOVERED: {len(links_without_proper_labels)} category links lack labels - Violates WCAG 2.1"
        )
...
def test_focus_indicators_visible_BR_015(browser):
    ...
    try:
        ...
        if not has_focus_indicator:
            logging.warning("=" * 80)
            logging.warning(
                "ACCESSIBILITY ISSUE: FOCUS INDICATORS MAY BE MISSING"
            )
            logging.warning(
                "Standard: WCAG 2.1 SC 2.4.7 (Focus Visible) - Level AA"
            )
            ...
            logging.warning(
                "  - Difficult to navigate for motor-impaired users"
            )
            logging.warning(
                "Recommendation: Ensure focus has visible outline (min 2px)"
            )
            logging.warning("=" * 80)
            ...
            logging.info(
                "DISCOVERED: Focus indicators visible - WCAG 2.1 Level AA compliant"
            )
