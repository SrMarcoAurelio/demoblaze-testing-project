...
def check_security_headers(browser):
    ...
    try:
        csp_meta = browser.find_elements(
...
def test_sql_injection_product_id_INJ_001(browser, sql_payload):
    ...
    logging.info("=" * 80)
    logging.info(
        f"TC-PRODUCT-SEC-INJ-001: Testing SQL injection with payload: {sql_payload}"
    )
    ...
    if sql_errors:
        ...
        pytest.fail(
            f"DISCOVERED: SQL Injection vulnerability in product ID parameter"
        )

    logging.info(
        f"SQL injection payload '{sql_payload}' did not trigger SQL errors"
    )
    assert True
...
def test_sql_injection_error_disclosure_INJ_002(browser):
    ...
    for payload in invalid_payloads:
        ...
        sql_errors = check_for_sql_error_indicators(page_source)
        if sql_errors:
            verbose_errors.append({"payload": payload, "errors": sql_errors})
    ...
    if verbose_errors:
        ...
        logging.error(
            f"Verbose errors triggered by {len(verbose_errors)} payloads"
        )
        for err in verbose_errors[:2]:
            logging.error(f"  Payload '{err['payload']}': {err['errors']}")
        logging.error(
            "Impact: Database structure disclosure aids further attacks"
        )
...
def test_xss_product_id_parameter_INJ_003(browser, xss_payload):
    ...
    logging.info("=" * 80)
    logging.info(
        f"TC-PRODUCT-SEC-INJ-003: Testing XSS with payload: {xss_payload}"
    )
...
def test_xss_product_description_stored_INJ_004(browser):
    ...
    logging.info("=" * 80)
    logging.info(
        "TC-PRODUCT-SEC-INJ-004: Testing for stored XSS in description"
    )
    ...
    try:
        ...
        try:
            ...
            if found_patterns:
                ...
                logging.error(
                    "Impact: Persistent XSS affects all users viewing product"
                )
                logging.error("=" * 80)
...
def test_idor_product_enumeration_IDOR_001(browser):
    ...
    logging.warning(
        "Note: For public e-commerce, this may be expected behavior"
    )
    ...
    if len(accessible_products) > 10:
        logging.warning(
            f"DISCOVERED: {len(accessible_products)} products enumerable via sequential IDs"
        )
...
def test_idor_invalid_product_id_IDOR_002(browser):
    ...
    logging.info("=" * 80)
    logging.info(
        "TC-PRODUCT-SEC-IDOR-002: Testing invalid product ID handling"
    )
    ...
    for invalid_id in invalid_ids:
        ...
        if found_indicators:
            verbose_errors.append(
                {"id": invalid_id, "indicators": found_indicators}
            )
    ...
    if verbose_errors:
        ...
        pytest.fail(
            f"DISCOVERED: Verbose error messages for {len(verbose_errors)} invalid IDs"
        )
...
def test_idor_negative_product_id_IDOR_003(browser):
    ...
    for neg_id in negative_ids:
        ...
        try:
            ...
            if name_element.is_displayed():
                product_name = name_element.text
                hidden_products.append({"id": neg_id, "name": product_name})
        except NoSuchElementException:
            pass
    ...
    if hidden_products:
        ...
        pytest.fail(
            f"DISCOVERED: {len(hidden_products)} hidden products accessible via negative IDs"
        )
...
def test_price_tampering_url_parameter_PRICE_001(browser):
    ...
    try:
        ...
        if price_manipulation_detected:
            pytest.fail(
                "DISCOVERED: Price tampering vulnerability via URL manipulation"
            )
...
def test_path_traversal_product_id_TRAV_001(browser, traversal_payload):
    ...
    logging.info("=" * 80)
    logging.info(
        f"TC-PRODUCT-SEC-TRAV-001: Testing path traversal: {traversal_payload}"
    )
        ...
        if session_cookies:
            for cookie in session_cookies:
                ...
                if not is_secure or not is_httponly:
                    logging.warning("=" * 80)
                    logging.warning(
                        "SESSION SECURITY ISSUE: INSECURE COOKIE FLAGS"
                    )
...
def test_csrf_add_to_cart_CSRF_001(browser):
    ...
    logging.info("=" * 80)
    logging.info(
        "TC-PRODUCT-SEC-CSRF-001: Testing CSRF protection on add to cart"
    )
    ...
    try:
        ...
        if not csrf_found:
            logging.warning("=" * 80)
            logging.warning(
                "POTENTIAL VULNERABILITY: NO CSRF PROTECTION DETECTED"
            )
            ...
            logging.warning("Impact:")
            logging.warning(
                "  - Forced actions on behalf of authenticated users"
            )
            logging.warning("  - Unwanted purchases")
            logging.warning("  - Cart manipulation")
            logging.warning(
                "Note: Further testing required to confirm vulnerability"
            )
            logging.warning("=" * 80)
...
def test_security_headers_product_page_HEAD_001(browser):
    ...
    try:
        ...
        if missing_headers:
            logging.warning("=" * 80)
            logging.warning(
                "SECURITY HARDENING ISSUE: MISSING SECURITY HEADERS"
            )
...
def test_information_disclosure_page_source_INFO_001(browser):
    ...
    logging.info("=" * 80)
    logging.info(
        "TC-PRODUCT-SEC-INFO-001: Checking for information disclosure"
    )
    ...
    try:
        ...
        if found_keywords:
            logging.warning("=" * 80)
            logging.warning(
                "INFORMATION DISCLOSURE: SENSITIVE KEYWORDS IN SOURCE"
            )
