...
def get_product_details(browser):
    ...
    try:
        details["description"] = browser.find_element(
            *PRODUCT_DESCRIPTION
        ).text
    except NoSuchElementException:
        pass
...
def test_navigate_to_product_from_catalog_FUNC_001(browser):
    ...
    assert (
        "prod.html" in current_url
    ), f"FAILED: Not on product page. URL: {current_url}"

    logging.info(
        f"SUCCESS: Navigated to product detail page for '{product_name}'"
    )
...
def test_product_name_displays_FUNC_002(browser):
    ...
    try:
        ...
        assert (
            product_name_element.is_displayed()
        ), "FAILED: Product name not visible"
...
def test_product_price_displays_FUNC_003(browser):
    ...
    try:
        ...
        assert (
            "$" in price_text or "dollar" in price_text.lower()
        ), f"FAILED: Price format unexpected: {price_text}"
...
def test_product_description_displays_FUNC_004(browser):
    ...
    try:
        ...
        assert (
            description_element.is_displayed()
        ), "FAILED: Description not visible"

        description_text = description_element.text
        assert len(description_text) > 0, "FAILED: Description is empty"

        logging.info(
            f"SUCCESS: Product description displays ({len(description_text)} characters)"
        )
...
def test_product_image_displays_FUNC_005(browser):
    ...
    try:
        ...
        assert (
            image_element.is_displayed()
        ), "FAILED: Product image not visible"

        image_src = image_element.get_attribute("src")
        assert image_src, "FAILED: Image src attribute is empty"
        assert image_src.startswith(
            "http"
        ), f"FAILED: Invalid image URL: {image_src}"
...
def test_add_to_cart_button_present_FUNC_006(browser):
    ...
    try:
        ...
        assert (
            add_to_cart_btn.is_displayed()
        ), "FAILED: Add to cart button not visible"
...
def test_add_to_cart_from_product_page_FUNC_007(browser):
    ...
    try:
        ...
        try:
            ...
            logging.info(
                f"SUCCESS: Add to cart triggered alert: '{alert_text}'"
            )
            assert (
                "added" in alert_text.lower() or "cart" in alert_text.lower()
...
def test_back_to_catalog_navigation_FUNC_008(browser):
    ...
    try:
        ...
        assert (
            ...
            or current_url == BASE_URL
            or current_url == BASE_URL.rstrip("/")
        ...
        assert (
            len(products) > 0
        ), "FAILED: No products visible after returning to catalog"

        logging.info(
            f"SUCCESS: Navigated back to catalog, {len(products)} products visible"
        )
...
def test_browser_back_button_FUNC_009(browser):
    ...
    assert (
        catalog_url != product_url
    ), "FAILED: Browser back did not change URL"
...
def test_multiple_product_navigation_FUNC_010(browser):
    ...
    assert (
        len(product_links) >= 2
    ), "FAILED: Not enough products to test navigation"
    ...
    logging.info(
        f"SUCCESS: Navigated to {len(visited_products)} different products"
    )
...
def test_all_products_have_name_BR_001(browser):
    ...
    if products_without_name:
        ...
        logging.error(
            f"Products without names: {len(products_without_name)}/{min(10, total_products)}"
        )
        logging.error(f"Product indices: {products_without_name}")
        logging.error(
            "Impact: Poor user experience, missing critical information"
        )
        logging.error("=" * 80)

        pytest.fail(
            f"DISCOVERED: {len(products_without_name)} products lack name information"
        )

    logging.info(
        f"SUCCESS: All tested products ({min(10, total_products)}) have names"
    )
...
def test_all_products_have_price_BR_002(browser):
    ...
    if products_without_price:
        ...
        logging.error(
            f"Products without prices: {len(products_without_price)}/{min(10, total_products)}"
        )
        ...
        pytest.fail(
            f"DISCOVERED: {len(products_without_price)} products lack price information"
        )

    logging.info(
        f"SUCCESS: All tested products ({min(10, total_products)}) have prices"
    )
...
def test_all_products_have_description_BR_003(browser):
    ...
    logging.info("=" * 80)
    logging.info(
        "TC-PRODUCT-BR-003: Validating all products have descriptions"
    )
    ...
    if products_without_description:
        ...
        logging.warning(
            f"Products without descriptions: {len(products_without_description)}/{min(10, total_products)}"
        )
        logging.warning(
            "Impact: Poor user experience, insufficient product information"
        )
        logging.warning("=" * 80)

        pytest.fail(
            f"DISCOVERED: {len(products_without_description)} products lack descriptions"
        )

    logging.info(
        f"SUCCESS: All tested products ({min(10, total_products)}) have descriptions"
    )
...
def test_all_product_images_load_successfully_BR_004(browser):
    ...
    logging.info("=" * 80)
    logging.info(
        "TC-PRODUCT-BR-004: Validating product images load successfully"
    )
        ...
        try:
            ...
            if image_src:
                if not check_image_loads_successfully(image_src):
                    broken_images.append({"index": i + 1, "url": image_src})
            else:
                broken_images.append(
                    {"index": i + 1, "url": "No src attribute"}
                )

        except NoSuchElementException:
            broken_images.append(
                {"index": i + 1, "url": "Image element not found"}
            )
    ...
    if broken_images:
        ...
        logging.error("CVSS Score: 5.3 (MEDIUM)")
        logging.error(
            f"Broken images: {len(broken_images)}/{min(10, total_products)}"
        )
        ...
        pytest.fail(
            f"DISCOVERED: {len(broken_images)} products have broken images"
        )

    logging.info(
        f"SUCCESS: All tested product images ({min(10, total_products)}) load successfully"
    )
...
def test_product_price_format_consistency_BR_005(browser):
    ...
    for i in range(min(10, total_products)):
        ...
        try:
            ...
            has_includes = (
                "includes" in price_text.lower()
                or "*includes" in price_text.lower()
            )
    ...
    if len(price_formats) > 1:
        ...
        pytest.fail(
            f"DISCOVERED: {len(price_formats)} different price formats found"
        )
...
def test_product_detail_load_time_BR_006(browser):
    ...
    for i in range(min(5, len(product_links))):
        ...
        if load_time > 3.0:
            slow_loads.append({"index": i + 1, "time": load_time})
    ...
    if slow_loads:
        ...
        logging.error("CVSS Score: 5.3 (MEDIUM)")
        logging.error(
            f"Products exceeding 3s: {len(slow_loads)}/{min(5, len(product_links))}"
        )
        for product in slow_loads:
            logging.error(
                f"  Product {product['index']}: {product['time']:.2f}s"
            )
        ...
        pytest.fail(
            f"DISCOVERED: {len(slow_loads)} products exceed 3s load time"
        )
...
def test_add_to_cart_button_visibility_BR_007(browser):
    ...
    try:
        ...
        if not is_displayed or not is_enabled:
            logging.error("=" * 80)
            logging.error(
                "USABILITY VIOLATION: ADD TO CART BUTTON NOT PROPERLY VISIBLE"
            )
            ...
            pytest.fail(
                "DISCOVERED: Add to cart button not properly visible or enabled"
            )
...
def test_product_image_has_alt_text_BR_008(browser):
    ...
    if images_without_alt:
        ...
        logging.error("CVSS Score: 7.5 (HIGH)")
        logging.error(
            f"Images without alt text: {len(images_without_alt)}/{min(5, len(product_links))}"
        )
        ...
        pytest.fail(
            f"DISCOVERED: {len(images_without_alt)} products lack image alt text (WCAG 2.1 Level A violation)"
        )
...
def test_keyboard_navigation_product_page_BR_009(browser):
    ...
    try:
        ...
        for _ in range(10):
            ...
            if (
                focused_element.tag_name == "a"
                and "btn-success" in focused_element.get_attribute("class")
            ):
                logging.info(
                    "SUCCESS: Add to cart button reachable via keyboard"
                )
                return
        ...
        pytest.fail(
            "DISCOVERED: Add to cart button not accessible via keyboard (WCAG 2.1 Level A violation)"
        )
...
def test_product_data_consistency_across_views_BR_010(browser, product_index):
    ...
    logging.info("=" * 80)
    logging.info(
        f"TC-PRODUCT-BR-010: Validating data consistency for product {product_index + 1}"
    )
    ...
    try:
        ...
        if catalog_name.strip().lower() != detail_name.strip().lower():
            ...
            pytest.fail(
                f"DISCOVERED: Product name inconsistent between catalog and detail page"
            )
