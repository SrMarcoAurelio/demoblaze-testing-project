...
def test_rapid_concurrent_login_attempts_BOT_002(browser):
    ...
    if successful_attempts >= 15:
        logging.warning(
            f"Concurrent brute force: {successful_attempts}/20 requests completed"
        )
...
def test_concurrent_session_handling_AUTH_003(browser):
    ...
    try:
        ...
        if still_logged_first:
            ...
            logging.info(
                "First session invalidated when second login occurred"
            )
...
def test_verbose_error_messages_INFO_001(browser):
    ...
    if alert_text:
        ...
        if found_keywords:
            ...
            pytest.fail(
                f"DISCOVERED: Verbose error message contains: {found_keywords}"
            )
...
def test_dangerous_http_methods_HTTP_001(browser):
    ...
    for method in dangerous_methods:
        try:
            response = requests.request(
                method, BASE_URL, timeout=TIMEOUT_SHORT
            )
    ...
    if allowed_methods:
        ...
        logging.warning(
            "Impact: Potential for data manipulation or information disclosure"
        )
        pytest.fail(
...
def test_clickjacking_protection_CLICK_001(browser):
    ...
    try:
        ...
        if iframe:
            ...
            pytest.fail(
                "DISCOVERED: Site vulnerable to clickjacking - no frame protection"
            )
    except:
    ...
    try:
        ...
        for weak_pass in weak_passwords:
            ...
            if alert_text and "success" in alert_text.lower():
                ...
                pytest.fail(
                    f"DISCOVERED: Weak password '{weak_pass}' was accepted"
                )
...
def test_remember_me_security_REMEM_001(browser):
    ...
    if not remember_me_exists:
        ...
        logging.info(
            "Note: This is acceptable for security-focused applications"
        )
    else:
